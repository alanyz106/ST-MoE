...
from Dataset_tools import Dataset_CHI3D as datasets
...
def main(opt):
    ...
    if opt.mode == 'train':
        dataset_mode = 0
        print('>>> DATA loading >>>')
        dataset = datasets.Datasets(opt, mode=dataset_mode)
        eval_dataset = datasets.Datasets(opt, mode=1)

        print('>>> Training dataset length: {:d}'.format(dataset.__len__()))
        data_loader = DataLoader(dataset, batch_size=opt.batch_size, shuffle=True, num_workers=0, pin_memory=False)
        eval_data_loader = DataLoader(eval_dataset, batch_size=opt.batch_size, shuffle=True, num_workers=0, pin_memory=False)
    elif opt.mode == 'test':
        dataset_mode = 1
        print('>>> DATA loading >>>')
        dataset = datasets.Datasets(opt, mode=dataset_mode)
